...
enum Phase { SelectUnit, SelectAction, SelectTarget, EnemyTurn, GameOver }
enum TileType { Floor, Obstacle, Water, Forest }
...
function knightTemplate(): Omit<Unit, 'id' | 'team' | 'pos'> {
  return {
    name: 'Knight',
    className: 'Knight',
    emoji: '⚔️',
...
function mageTemplate(): Omit<Unit, 'id' | 'team' | 'pos'> {
  return {
    name: 'Mage',
    className: 'Mage',
    emoji: '🔮',
...
function archerTemplate(): Omit<Unit, 'id' | 'team' | 'pos'> {
  return {
    name: 'Archer',
    className: 'Archer',
    emoji: '🏹',
...
function rogueTemplate(): Omit<Unit, 'id' | 'team' | 'pos'> {
  return {
    name: 'Rogue',
    className: 'Rogue',
    emoji: '🗡️',
...
function generateMap(): TileType[][] {
  ...
  for (const o of obstacles) map[o.y][o.x] = TileType.Obstacle;

  // water
  const water: Pos[] = [{x:4,y:4},{x:5,y:4},{x:4,y:3},{x:5,y:3}];
  for (const w of water) map[w.y][w.x] = TileType.Water;

  // forest
  const forests: Pos[] = [{x:1,y:2},{x:2,y:2},{x:7,y:5},{x:8,y:5},{x:6,y:1}];
  for (const f of forests) map[f.y][f.x] = TileType.Forest;
...
let nextId = 1;
function makeUnit(template: Omit<Unit, 'id' | 'team' | 'pos'>, team: Team, pos: Pos): Unit {
...
function isWalkable(state: GameState, p: Pos): boolean {
  if (!inBounds(p)) return false;
  const tile = state.map[p.y][p.x];
  if (tile === TileType.Obstacle || tile === TileType.Water) return false;
  return !state.units.some(u => u.hp > 0 && posEq(u.pos, p));
}

function getMovementRange(state: GameState, unit: Unit): Pos[] {
  const result: Pos[] = [];
  ...
  while (queue.length > 0) {
    ...
    if (cur.cost > 0) result.push(cur.pos);
    if (cur.cost >= speed) continue;
    for (const d of [{ x: 0, y: -1 }, { x: 0, y: 1 }, { x: -1, y: 0 }, { x: 1, y: 0 }]) {
      const np: Pos = { x: cur.pos.x + d.x, y: cur.pos.y + d.y };
      const key = `${np.x},${np.y}`;
...
function getAttackRange(state: GameState, unit: Unit): Pos[] {
  const result: Pos[] = [];
  for (let y = 0; y < GRID_ROWS; y++) {
    for (let x = 0; x < GRID_COLS; x++) {
      const p = { x, y };
...
function getAbilityRange(state: GameState, unit: Unit, ability: Ability): Pos[] {
  const result: Pos[] = [];
...
function addFloatingText(state: GameState, text: string, pos: Pos, color: string) {
...
function performAttack(state: GameState, attacker: Unit, target: Unit) {
...
function performAbility(state: GameState, caster: Unit, targetPos: Pos, ability: Ability) {
...
function doEnemyAction(state: GameState, enemy: Unit) {
  ...
  for (const pu of playerUnits) {
    ...
    if (d < nearestDist) { nearest = pu; nearestDist = d; }
  }
...
function handleClick(state: GameState, canvasX: number, canvasY: number) {
  ...
  if (!inBounds({ x: tileX, y: tileY })) return;
  const clickPos: Pos = { x: tileX, y: tileY };
...
function checkAllDone(state: GameState) {
  if (state.phase === Phase.GameOver) return;
  const playerUnits = state.units.filter(u => u.team === Team.Player && u.hp > 0);
...
function drawTile(ctx: CanvasRenderingContext2D, x: number, y: number, tileType: TileType) {
...
function drawUnit(ctx: CanvasRenderingContext2D, unit: Unit, isSelected: boolean) {
  if (unit.hp <= 0) return;
...
function drawHighlights(ctx: CanvasRenderingContext2D, tiles: Pos[], color: string) {
...
function drawButton(ctx: CanvasRenderingContext2D, btn: UIButton, accent = false) {
...
function drawHover(ctx: CanvasRenderingContext2D, state: GameState) {
